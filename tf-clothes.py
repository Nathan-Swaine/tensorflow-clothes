...
fashion_mnist = tf.keras.datasets.fashion_mnist #specifically import the 'fashion data' from the mnist general data set import and load the data from tensor flow there are 60k training images and 10k test images
(train_images, train_labels), (test_images, test_labels) = fashion_mnist.load_data()
class_names = ['T-shirt/top', 'Trouser', 'Pullover', 'Dress', 'Coat', 'Sandal', 'Shirt', 'Sneaker', 'Bag', 'Ankle boot'] #classifacations for different images, must align with train images and train images labels
train_images = train_images /255
test_images = test_images /255#its important the test data is treat EXACTLY like the train data 
...
model.fit(train_images, train_labels, epochs=20) # fits the model to the training data
